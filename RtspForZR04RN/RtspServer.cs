...
{
    const uint global_ssrc = 0x04040404; // 8 hex digits

    private TcpListener rtspServerListener;
    bool stopping;
    ...
    async Task UpdateClients()
    {
        ...
        else if (count > 0 && lc == 0)
        {
            await nvr.Connect(nvrHost, nvrPort);
            await nvr.Login(nvrUsername, nvrPassword);
            streamId = await nvr.StreamStart(nvrChannel);
        }
    }
    ...
    private void RTSP_Message_Received(object sender, RtspChunkEventArgs e)
    {
        ...
        if (message is Rtsp.Messages.RtspRequestDescribe)
        {
            ...
            sdp.Append("v=0\n");
            sdp.Append("o=admin 0 IN IP4 0.0.0.0\n");
            sdp.Append("s=RTSP for ZR04RN\n");
            sdp.Append("m=video 0 RTP/AVP 96\n");
            sdp.Append("c=IN IP4 0.0.0.0\n");
            sdp.Append("a=control:trackID=0\n");
            sdp.Append("a=rtpmap:96 H264/90000\n");
            // sdp.Append("a=fmtp:96 profile-level-id=42A01E;");
        ...
        if (message is Rtsp.Messages.RtspRequestSetup)
        {
            ...
            {
                bool udp_supported = true; // DISABLE UDP
                if (udp_supported)
                {
                    // RTP over UDP mode
    ...
    public class RTSPConnection
    {
        ...
        public ushort video_sequence_number = 1;         // 16 bit RTP packet sequence number used with this client connection
